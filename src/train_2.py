...
def load_model_and_tokenizer(config: dict):
    ...
    # Only print trainable params on the main process to avoid spam
...
def train(config_path: str):
    ...
    # --- LOGGING FIX: Silence Loguru on worker processes ---
    if global_rank != 0:
        logger.remove()
        logger.add(sys.stderr, level="ERROR") # Keep CRITICAL errors visible
    ...
    # --- CALLBACK FIX: Only run S3 upload on the main process ---
    trainer_callbacks = []
    if global_rank == 0:
        ...
        trainer_callbacks.append(s3_callback)
    ...
    # --- WANDB FIX: Only finish on main process ---
    if global_rank == 0:
        wandb.finish()
